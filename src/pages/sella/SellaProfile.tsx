...
export default function SellaProfile() {
  const { user, logout } = useAuth();
  const { theme } = useTheme();
  const navigate = useNavigate();
  const [showLogout, setShowLogout] = useState(false);

  const handleLogout = () => { logout(); navigate("/a/signin"); };

  const themeLabel = theme === "dark" ? "Dark" : theme === "light" ? "Light" : "System";

  const accountItems = [
    { icon: User, label: "KYC Verification", onClick: () => navigate("/a/kyc"), right: <span className="text-xs bg-success/20 text-success px-2 py-0.5 rounded">Verified</span> },
    { icon: Landmark, label: "Bank Details", onClick: () => navigate("/a/bank-details") },
    { icon: Bell, label: "Notifications", onClick: () => navigate("/a/notifications") },
  ...
  const securityItems = [
    { icon: Lock, label: "Update Password", onClick: () => navigate("/a/update-password") },
    { icon: Grid3X3, label: "Update Pin", onClick: () => navigate("/a/update-pin") },
    { icon: Shield, label: "Two Factor Authentication", onClick: () => navigate("/a/two-factor") },
  ];

  const otherItems = [
    { icon: theme === "dark" ? Moon : Sun, label: "Appearance", onClick: () => navigate("/a/appearance"), right: <span className="text-xs bg-secondary text-muted-foreground px-2 py-0.5 rounded capitalize">{themeLabel}</span> },
    { icon: Gift, label: "Refer and Earn", onClick: () => navigate("/a/referral") },
    { icon: HelpCircle, label: "Support", onClick: () => navigate("/a/support") },
  ];
    ...
    <PageTransition>
      <div className="pt-4 pb-24">
        ...
        <div className="bg-card rounded-2xl p-4 flex items-center gap-4 mb-6 border border-border/20">
          ...
          <button onClick={() => navigate("/a/edit-profile")} className="text-xs text-primary border border-primary/30 px-3 py-1.5 rounded-lg flex items-center gap-1">
            <Pen size={10} /> Edit
          </button>
        </div>
