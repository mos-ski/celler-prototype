...
export default function SellaSignUp() {
  ...
  if (isLoggedIn && step !== "success") return <Navigate to="/a/home" replace />;
      ...
      <div className="flex min-h-screen flex-col px-6 pt-14 pb-8 bg-background">
        <div className="flex flex-col items-center justify-center flex-1">
          ...
          <p className="text-muted-foreground text-center mt-2 max-w-xs">
            Your account has been created and you will receive an email notification shortly
          </p>
          <Button onClick={() => navigate("/a/home", { replace: true })} className="w-full h-14 rounded-2xl text-base font-semibold mt-10 bg-primary text-primary-foreground">
            Continue
          </Button>
        </div>
      </div>
      ...
      {step === "email" && (
        <div className="flex flex-col flex-1">
          ...
          <p className="text-center text-sm text-muted-foreground mt-auto pt-8">
            Already a user? <Link to="/a/signin" className="text-primary font-medium">Sign in</Link>
          </p>
        </div>
      )}
